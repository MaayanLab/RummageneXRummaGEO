...
def fisher(gene_set, library, fisher=None, min_set_size=5, min_overlap=2, uppercase=True, background_size=22000, verbose=True):
    ...
    if is_library(gene_set):
        ...
        for k in tqdm.tqdm(gene_set, disable=not verbose):
            ...
            df = fastfisher(temp_gs, vec_lib, lib_length, fisher=fisher, min_overlap=min_overlap, min_set_size=min_set_size, background_size=background_size, verbose=verbose)
            df["rummagene"] = [k for ele in range(len(df))]
            df['rummagene-size'] = [len(temp_gs) for ele in range(len(df))]
            enrichments = pd.concat([enrichments, df], ignore_index=True)
        return enrichments
...
def fastfisher(gene_set, library, set_sizes, fisher=None, min_set_size=5, min_overlap=2, background_size=22000, verbose=True):
    ...
    result = pd.DataFrame({
        "rummageo": np.array(library_keys, dtype=object)[relevant_idx],
        "p-value": np.array(p_values, dtype='float64'),
        "sidak": np.array(sidak_corrected_pvals, dtype='float64'),
        "fdr": np.array(fdr_corrected_pvals, dtype='float64'),
        "odds": np.array(odds, dtype='float64'),
        "n-overlap": a[relevant_idx], 
        "rummageo-size": set_sizes[relevant_idx].astype('int32')
    })
    result.sort_values(by=["p-value", "n-overlap"], ascending=[True, False],inplace=True)
    result  = result.loc[result['p-value'] <= 0.001]
