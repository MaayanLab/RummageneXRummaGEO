...
import { useTermsPmcsQuery , useTermsPmcs4Query} from '@/graphql'
import PmcSearchData from '@/components/pmcSearchData'
...
function PubMedSearchResults({ search }: { search: string }) {
  const { data: pmcData, error:errore, isLoading } = useSWR(() => search, pubmed_search)
  const pmcCount = React.useMemo(() => pmcData?.esearchresult?.count !== undefined ? Number(pmcData?.esearchresult?.count) : 0, [pmcData])

  const { data: y, error, loading } = useTermsPmcs4Query({
    variables: { term: search }
  });
  const pmcids = y?.termsPmcsArray ?? [];
  // const pmcCount = 5000;

  const { data, loading: termsLoading } = useTermsPmcsQuery({
    skip: pmcids.length === 0,
    variables: { pmcids }
  });

  const pmcsInDb = useMemo(() => y?.termsPmcsArray, [y]);

  const { pmc_terms, gene_set_ids } = useMemo(() => {
  ...
  if (error) {
    return (
      <div className="text-center p-5">
        ...
        Failed to fetch articles from PubMed Central... trying again in a few seconds.
      </div>
    );
  }
  
  if (loading || termsLoading || isLoading) return <Loading />;

  if (!pmcsInDb) return null;

  if (pmcsInDb.length < 1) {
    return (
      <div className="text-center p-5">
        Your query returned {Intl.NumberFormat("en-US", {}).format(pmcCount)} articles, but none of them are contained in the RummagenexRummaGEO database. Please try refining your query.
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-2 my-2">
      <h2 className="text-md font-bold">
        Your query returned {Intl.NumberFormat("en-US", {}).format(pmcCount)} articles from PubMed Central.{" "}
        {pmcCount > 5000 ? (
          <>
            Since there are more than 5,000 papers that match your query, we only display{" "}
            {Intl.NumberFormat("en-US", {}).format(gene_set_ids.size)} gene sets from{" "}
            {Intl.NumberFormat("en-US", {}).format(pmc_terms.size)} publications containing gene sets from the first
            5,000 publications returned from your query. Please narrow your search to obtain better results.
          </>
        ) : (
          <>
            RummagenexRummaGEO{" "}
            <Image
              ...
              src="/images/rummagenexrummageo_logo.png"
              width={50}
              height={100}
              alt="RummagenexRummaGEO"
            />{" "}
            found {Intl.NumberFormat("en-US", {}).format(gene_set_ids.size)} gene sets from{" "}
            {Intl.NumberFormat("en-US", {}).format(pmc_terms.size)} publications containing gene sets from the
            publications returned from your query.
          </>
        )}
      </h2>
      <PmcSearchData pmc_terms={pmc_terms} pmcs={pmcsInDb} gene_set_ids={gene_set_ids}></PmcSearchData>
    </div>
  );
}
const examples = [
